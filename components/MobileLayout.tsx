...
const ALL_ITEMS = [
  { href: "/dashboard",             label: "Dashboard",       icon: LayoutDashboard, color: "bg-blue-500",    textColor: "text-blue-600" },
  { href: "/dashboard/dairelerim",  label: "Dairelerim",      icon: Building2,       color: "bg-emerald-500", textColor: "text-emerald-600" },
  { href: "/dashboard/gelirler",    label: "Kira Gelirleri",  icon: TrendingUp,      color: "bg-green-500",   textColor: "text-green-600" },
  { href: "/dashboard/sozlesmeler", label: "Sözleşmeler",     icon: FileText,        color: "bg-violet-500",  textColor: "text-violet-600" },
  { href: "/dashboard/bakim",       label: "Bakım Talepleri", icon: Wrench,          color: "bg-orange-500",  textColor: "text-orange-600" },
  { href: "/dashboard/belgeler",    label: "Belgeler",        icon: FolderOpen,      color: "bg-yellow-500",  textColor: "text-yellow-600" },
  { href: "/dashboard/sifre",       label: "Şifre Değiştir",  icon: KeyRound,        color: "bg-gray-500",    textColor: "text-gray-600" },
];
...
export default function MobileLayout({ ad, soyad, children }: { ad: string; soyad: string; children: React.ReactNode }) {
  ...
  const baslik = aktifItem?.label ?? "Ev Sahibi";
  ...
  return (
    <div className="md:hidden flex flex-col h-screen bg-gray-50 overflow-hidden">
      <header className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white px-3 pt-[env(safe-area-inset-top)] shrink-0 shadow-md">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-[0_-2px_10px_rgba(0,0,0,0.04)] pb-[env(safe-area-inset-bottom)]">
        <div className="grid grid-cols-5 h-16">
          ...
          <button onClick={() => setMenuAcik(true)} className={`flex flex-col items-center justify-center gap-0.5 text-[10px] transition-colors ${menuAcik ? "text-blue-600 font-semibold" : "text-gray-500"}`}>
          ...
          <div onClick={(e) => e.stopPropagation()} className="relative bg-white rounded-t-3xl shadow-2xl pb-[env(safe-area-inset-bottom)] animate-slide-up">
            ...
            <div className="grid grid-cols-3 gap-3 p-4 pt-2">
              {ALL_ITEMS.map(({ href, label, icon: Icon, color }) => {
                const active = pathname === href;
                return (
                  <Link key={href} href={href} onClick={() => setMenuAcik(false)} className={`flex flex-col items-center gap-1.5 p-3 rounded-2xl transition-colors ${active ? "bg-blue-50" : "hover:bg-gray-50"}`}>
      ...
      {drawerAcik && (
        <div className="fixed inset-0 z-50 flex" onClick={() => setDrawerAcik(false)}>
          ...
          <aside onClick={(e) => e.stopPropagation()} className="relative w-72 max-w-[85%] h-full bg-gray-900 text-white flex flex-col shadow-2xl animate-slide-right">
            <div className="bg-gradient-to-br from-blue-600 to-indigo-800 px-5 pt-[calc(env(safe-area-inset-top)+1rem)] pb-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-white/15 flex items-center justify-center"><User size={18} /></div>
                  <div><p className="font-semibold text-sm">{ad} {soyad}</p><p className="text-[11px] text-blue-100">Ev Sahibi</p></div>
                </div>
            ...
            <nav className="flex-1 overflow-y-auto px-3 py-3 space-y-0.5">
              {ALL_ITEMS.map(({ href, label, icon: Icon, color }) => {
                const active = pathname === href;
                return (
                  <Link key={href} href={href} onClick={() => setDrawerAcik(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${active ? "bg-blue-600 text-white" : "text-gray-300 hover:bg-gray-800 hover:text-white"}`}>
